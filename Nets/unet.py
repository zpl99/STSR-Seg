...
class U_Net(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=1, image_size = 128):
        ...
        self.Conv = nn.Conv2d(filters[0], 4, kernel_size=1, stride=1, padding=0)

        # self.var_layer = nn.Conv2d(4, 1, kernel_size=3, stride=2, padding=1) # TODO: 如果不包含var路径则把它注释掉，否则load会报错
    ...
    def forward(self, x):
        ...
        x = self.firstbn(x)
        X_00 = self.firstrelu(x)  # 128,H/2,W/2
        # Encoder
        x = self.firstmaxpool(X_00)  # 128,H/4,W/4
        X_10 = self.encoder1(x)  # 256,H/4,W/4
        X_20 = self.encoder2(X_10)  # 512,H/8,W/8
        X_30 = self.encoder3(X_20)  # 1024,H/16,W/16
        X_40 = self.encoder4(X_30)  # 2048,H/32,W/32

        d5 = self.Up5(X_40)
        d5 = torch.cat((X_30, d5), dim=1)
        d5 = self.Up_conv5(d5)
        d5 =self.relu5(d5)
        d5 = self.bn5(d5)

        d4 = self.Up4(d5)
        d4 = torch.cat((X_20, d4), dim=1)
        d4 = self.Up_conv4(d4)
        d4 =self.relu4(d4)
        d4 = self.bn4(d4)

        d3 = self.Up3(d4)
        d3 = torch.cat((X_10, d3), dim=1)
        d3 = self.Up_conv3(d3)
        d3 =self.relu3(d3)
        d3 = self.bn3(d3)

        d2 = self.Up2(d3)
        d2 = torch.cat((X_00, d2), dim=1)
        d2 = self.Up_conv2(d2)
        out = self.Conv(d2)
...
class U_Net_wTC(nn.Module):
    ...
    def __init__(self, in_ch=3, out_ch=1, image_size=128):
        ...
        for n, m in self.encoder4.named_modules():
            if 'conv2' in n:
                ...
                m.stride = (2, 2)
        self.TP_embedding = nn.Sequential(nn.AdaptiveMaxPool2d((1, 1)), nn.Flatten(), nn.Linear(2048, 128),
    ...
    def forward(self, x):
        ...
        x_embedding = self.TP_embedding(X_40)
        d5 = self.Up5(X_40)
        d5 = torch.cat((X_30, d5), dim=1)
        d5 = self.Up_conv5(d5)
        d5 = self.relu5(d5)
