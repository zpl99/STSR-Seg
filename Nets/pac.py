...
class CosineFn(Function):
    ...
    def forward(ctx, input, kernel_size, stride, padding, dilation, channel_wise):
        ...
        feat_0 = cols.contiguous()[:, :, center_y:center_y + 1, center_x:center_x + 1, :, :]  # 中心点(如果是3x3，那2，2就是中心点)
        # diff_sq = (cols - feat_0).pow(2)
        consine_sq = -1 * F.normalize(cols, dim=[-2, -1]) * F.normalize(feat_0,
                                                                        dim=[-2,
                                                                             -1])  # TODO: dim=[2,3] or dim=[-2,-1] ?
        output = torch.div(consine_sq, cols.pow(2))
...
def packernel2d(input, mask=None, kernel_size=0, stride=1, padding=0, output_padding=0, dilation=1,
    ...
    if native_impl:
        ...
        assert (smooth_kernel_type == 'none' and
                kernel_type in __kernelFn__)
        if kernel_type == "cosine":
            output = CosineFn.apply(input, kernel_size, stride, padding, dilation, channel_wise)
        else:
...
class _PacConvNd(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride,
        ...
        elif kernel_type not in __kernelFn__:
...
if __name__ == '__main__':
    in_ch, out_ch, g_ch = 256, 256, 3  # channel sizes of input, output and guidance
    f, b, h, w = 3, 16, 64, 64  # filter size, batch size, input height and width
    input = torch.ones(b, in_ch, h, w)
    guide = torch.ones(b, g_ch, h, w)  # guidance feature ('f' in Eq.3 of paper)

    conv = nn.Conv2d(in_ch, out_ch, f)
    out_conv = conv(input)  # standard spatial convolution

    pacconv = PacConv2d(in_ch, out_ch, f, kernel_type="cosine", padding=1)
