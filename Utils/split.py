...
class EMPatches(object):
    ...
    def extract_patches(self, img, patchsize, overlap=None, stride=None):
        ...
        indices = self.get_indices(height * 4, width * 4, patchsize=256, overlap=0.1)
        return img_patches, indices

    def get_indices(self, height, width, patchsize, overlap=None, stride=None):
    ...
    def merge_patches(self, img_patches, indices, mode='overwrite'):
        ...
        if mode == 'min':
            if rgb:
                empty_image = np.zeros((orig_h, orig_w, 4)).astype(np.float32) + np.inf  # using float here is better
            else:
                empty_image = np.zeros((orig_h, orig_w)).astype(np.float32) + np.inf  # using float here is better
        else:
            if rgb:
                empty_image = np.zeros((orig_h, orig_w, 4)).astype(np.float32)  # using float here is better
            else:
        ...
        for i, indice in enumerate(indices):
            if mode == 'overwrite':
                ...
                if rgb:
                    empty_image[indice[0]:indice[1], indice[2]:indice[3], :] = np.maximum(img_patches[i], empty_image[
                                                                                                          indice[0]:
                                                                                                          indice[1],
                                                                                                          indice[2]:
                                                                                                          indice[3], :])
                else:
                    empty_image[indice[0]:indice[1], indice[2]:indice[3]] = np.maximum(img_patches[i],
                                                                                       empty_image[indice[0]:indice[1],
                                                                                       indice[2]:indice[3]])
            elif mode == 'min':
                if rgb:
                    empty_image[indice[0]:indice[1], indice[2]:indice[3], :] = np.minimum(img_patches[i], empty_image[
                                                                                                          indice[0]:
                                                                                                          indice[1],
                                                                                                          indice[2]:
                                                                                                          indice[3], :])
                else:
                    empty_image[indice[0]:indice[1], indice[2]:indice[3]] = np.minimum(img_patches[i],
                                                                                       empty_image[indice[0]:indice[1],
                                                                                       indice[2]:indice[3]])
            elif mode == 'avg':
                if rgb:
